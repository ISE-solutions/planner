...
export const formatActivityRows = ({
  activities,
  dictTag,
  dictPeople,
  dictSpace,
  ...
  Object.keys(groupedActivities).forEach((key, i) => {
    ...
    actvArray.forEach((elm) => {
      const per =
        dictPeople?.[elm?.[`${PREFIX}pessoa`]]?.[`${PREFIX}nomecompleto`];
      const func = dictTag?.[elm?.[`${PREFIX}funcao`]]?.[`${PREFIX}nome`];

      people.push(`${per || ''} ${func ? `(${func})` : ''}`);

      const fullSpace = dictSpace?.[elm?.[`${PREFIX}espacoid`]];
      const capacity = fullSpace?.[`${PREFIX}Espaco_CapacidadeEspaco`].find(
        (cap) =>
          dictTag?.[cap?.[`_${PREFIX}uso_value`]]?.[`${PREFIX}nome`] ===
          EFatherTag.ALERTA
      );
      if (fullSpace) {
        spaces.push(
          `${fullSpace?.[`${PREFIX}nome`]}(${
            capacity?.[`${PREFIX}quantidade`] || ''
          })`
        );
      }

      document.push(elm?.[`documento_nome`]);

      const perTeam =
        dictPeople?.[elm?.[`turma_pessoa`]]?.[`${PREFIX}nomecompleto`];
      const funcTeam = dictTag?.[elm?.[`turma_funcao`]]?.[`${PREFIX}nome`];

      peopleTeam.push(`${perTeam || ''} ${funcTeam ? `(${funcTeam})` : ''}`);

      const perProgram =
        dictPeople?.[elm?.[`programa_pessoa`]]?.[`${PREFIX}nomecompleto`];
      const funcProgram =
        dictTag?.[elm?.[`programa_funcao`]]?.[`${PREFIX}nome`];

      peopleProgram.push(
        `${perProgram || ''} ${funcProgram ? `(${funcProgram})` : ''}`
      );
      ...
      peopleSchedule.push(
        `${perProgram || ''} ${perSchedule ? `(${funcSchedule})` : ''}`
      );

      participantTeam.push(
        `${
          elm.participante_data
    ...
    activitiesResult.push({
      ...
      selectRow: (
        <FormControlLabel
          value={rowsSelectedToExport.includes(i)}
          control={
            <Checkbox
              color='primary'
              checked={rowsSelectedToExport.includes(i)}
              value={rowsSelectedToExport.includes(i)}
      ...
      start: moment(firstActv?.[`${PREFIX}datahorainicio`] + 'Z').format(
        'DD/MM/YYYY HH:mm'
      ),
      dateSchedule: moment(firstActv?.[`${PREFIX}data`]).format('DD/MM/YYYY'),
      programName:
        dictTag[firstActv?.[`${PREFIX}nomeprograma`]]?.[`${PREFIX}nome`],
      end: moment(firstActv?.[`${PREFIX}datahorafim`] + 'Z').format(
        'DD/MM/YYYY HH:mm'
      ),
      [`${PREFIX}AreaAcademica`]:
        dictTag[firstActv?.[`${PREFIX}areaacademica`]],
      [`${PREFIX}Temperatura`]: dictTag[firstActv?.[`${PREFIX}temperatura`]],
      schedule: {
        [`${PREFIX}Modulo`]: dictTag[firstActv?.[`${PREFIX}modulo`]],
        [`${PREFIX}link`]: firstActv?.[`${PREFIX}link`],
        [`${PREFIX}linkbackup`]: firstActv?.[`${PREFIX}linkbackup`],
        [`${PREFIX}Modalidade`]: dictTag[firstActv?.[`modalidade_dia`]],
        [`${PREFIX}Ferramenta`]: dictTag[firstActv?.[`${PREFIX}ferramenta`]],
        [`${PREFIX}Temperatura`]:
          dictTag[firstActv?.[`cronograma_temperatura`]],
        [`${PREFIX}FerramentaBackup`]:
          dictTag[firstActv?.[`${PREFIX}ferramentabackup`]],
      },
      team: {
        [`${PREFIX}nome`]: firstActv?.[`turma_nome`],
        [`${PREFIX}sigla`]: firstActv?.[`${PREFIX}sigla`],
        [`${PREFIX}codigodaturma`]: firstActv?.[`${PREFIX}codigodaturma`],
        [`${PREFIX}nomefinanceiro`]: firstActv?.[`${PREFIX}nomefinanceiro`],
        [`${PREFIX}mascara`]: firstActv?.[`${PREFIX}mascara`],
        [`${PREFIX}mascarabackup`]: firstActv?.[`${PREFIX}mascarabackup`],
        [`${PREFIX}anodeconclusao`]: firstActv?.[`${PREFIX}anodeconclusao`],
        [`${PREFIX}Modalidade`]: dictTag[firstActv?.[`turma_modalidade`]],
        [`${PREFIX}Temperatura`]: dictTag[firstActv?.[`turma_temperatura`]],
      },
      program: {
        [`${PREFIX}NomePrograma`]:
          dictTag[firstActv?.[`${PREFIX}nomeprograma`]],
        [`${PREFIX}sigla`]: firstActv?.[`programa_sigla`],
        [`${PREFIX}TipoPrograma`]:
          dictTag[firstActv?.[`${PREFIX}tipoprograma`]],
        [`${PREFIX}Instituto`]: dictTag[firstActv?.[`${PREFIX}instituto`]],
        [`${PREFIX}Empresa`]: dictTag[firstActv?.[`${PREFIX}empresa`]],
        [`${PREFIX}Temperatura`]: dictTag[firstActv?.[`programa_temperatura`]],
      },
      ...
      participantTeam: _.uniq(participantTeam)
        .filter((e) => e?.trim())
        .join(', '),
      academicRequest: _.uniq(requestAcademic)
        .filter((e) => e?.trim())
        .join(', '),
      peopleSchedule: _.uniq(peopleSchedule)
        .filter((e) => e?.trim())
        .join(', '),
      people: _.uniq(people)
        .filter((e) => e?.trim())
        .join(', '),
      peopleTeam: _.uniq(peopleTeam)
        .filter((e) => e?.trim())
        .join(', '),
...
export const formatScheduleRows = ({
  schedules,
  dictTag,
  dictPeople,
  rowsSelectedToExport,
  ...
  const groupedSchedules = _.groupBy(
    newSchedules,
    (it) => it?.[`${PREFIX}cronogramadediaid`]
  );
  ...
  Object.keys(groupedSchedules).forEach((key, i) => {
    ...
    scheduleResult.push({
      ...
      dateSchedule: moment(firstItem?.[`${PREFIX}data`]).format('DD/MM/YYYY'),
      programName:
        dictTag[firstItem?.[`${PREFIX}nomeprograma`]]?.[`${PREFIX}nome`],
      schedule: {
        [`${PREFIX}Modulo`]: dictTag[firstItem?.[`${PREFIX}modulo`]],
        [`${PREFIX}link`]: firstItem?.[`${PREFIX}link`],
        [`${PREFIX}linkbackup`]: firstItem?.[`${PREFIX}linkbackup`],
        [`${PREFIX}Modalidade`]: dictTag[firstItem?.[`modalidade_dia`]],
        [`${PREFIX}Ferramenta`]: dictTag[firstItem?.[`${PREFIX}ferramenta`]],
        [`${PREFIX}Temperatura`]:
          dictTag[firstItem?.[`cronograma_temperatura`]],
        [`${PREFIX}FerramentaBackup`]:
          dictTag[firstItem?.[`${PREFIX}ferramentabackup`]],
      },
      team: {
        [`${PREFIX}nome`]: firstItem?.[`turma_nome`],
        [`${PREFIX}sigla`]: firstItem?.[`${PREFIX}sigla`],
        [`${PREFIX}codigodaturma`]: firstItem?.[`${PREFIX}codigodaturma`],
        [`${PREFIX}nomefinanceiro`]: firstItem?.[`${PREFIX}nomefinanceiro`],
        [`${PREFIX}mascara`]: firstItem?.[`${PREFIX}mascara`],
        [`${PREFIX}mascarabackup`]: firstItem?.[`${PREFIX}mascarabackup`],
        [`${PREFIX}anodeconclusao`]: firstItem?.[`${PREFIX}anodeconclusao`],
        [`${PREFIX}Modalidade`]: dictTag[firstItem?.[`turma_modalidade`]],
        [`${PREFIX}Temperatura`]: dictTag[firstItem?.[`turma_temperatura`]],
      },
      program: {
        [`${PREFIX}NomePrograma`]:
          dictTag[firstItem?.[`${PREFIX}nomeprograma`]],
        [`${PREFIX}sigla`]: firstItem?.[`programa_sigla`],
        [`${PREFIX}TipoPrograma`]:
          dictTag[firstItem?.[`${PREFIX}tipoprograma`]],
        [`${PREFIX}Instituto`]: dictTag[firstItem?.[`${PREFIX}instituto`]],
        [`${PREFIX}Empresa`]: dictTag[firstItem?.[`${PREFIX}empresa`]],
        [`${PREFIX}Temperatura`]: dictTag[firstItem?.[`programa_temperatura`]],
      },
      participantTeam: _.uniq(participantTeam)
        .filter((e) => e?.trim())
        .join(', '),
...
export const formatTeamRows = ({
  ...
  const groupedTeams = _.groupBy(newTeams, (it) => it?.[`${PREFIX}turmaid`]);
...
export const formatProgramRows = ({
  programs,
  dictTag,
  dictPeople,
  rowsSelectedToExport,
