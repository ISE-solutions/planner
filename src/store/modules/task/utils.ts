...
export const buildQuery = (filtro: IFilterProps) => {
  var query = new QueryBuilder().filter((f) => {
    ...
    filtro.delivery &&
      f.filterPhrase(
        `contains(${PREFIX}Entrega/${PREFIX}titulo,'${filtro.delivery}')`
      );

    if (filtro?.responsible && filtro.responsible.length) {
      f.or((p) => {
        filtro.responsible.forEach((st) => {
          p.filterPhrase(
            `${PREFIX}tarefas_responsaveis_ise_pessoa/any(c: c/${PREFIX}pessoaid eq '${st}')`
          );
        });
    ...
    if (filtro.start && filtro.end) {
      f.filterPhrase(
        `createdon gt '${filtro.start}' and createdon lt '${filtro.end}'`
      );
    }

    if (filtro.endForecastConclusion) {
      f.filterPhrase(
        `${PREFIX}previsaodeconclusao lt '${filtro.endForecastConclusion}'`
      );
    }
    ...
    filtro.teamSigla &&
      f.filterPhrase(
        `contains(${PREFIX}Turma/${PREFIX}sigla,'${filtro.teamSigla}')`
      );

    if (filtro?.teamTemperatureId && filtro.teamTemperatureId.length) {
      f.or((p) => {
        filtro.teamTemperatureId.forEach((st) => {
          p.filterExpression(
            `${PREFIX}Turma/_${PREFIX}temperatura_value`,
            'eq',
            st
          );
        });
    ...
    filtro.teamName &&
      f.filterPhrase(
        `contains(${PREFIX}Turma/${PREFIX}nome,'${filtro.teamName}')`
      );
    ...
    if (filtro?.instituteId && filtro.instituteId.length) {
      f.or((p) => {
        filtro.instituteId.forEach((st) => {
          p.filterExpression(
            `${PREFIX}Programa/_${PREFIX}instituto_value`,
            'eq',
            st
          );
        });
    ...
    if (filtro?.companyId && filtro.companyId.length) {
      f.or((p) => {
        filtro.companyId.forEach((st) => {
          p.filterExpression(
            `${PREFIX}Programa/_${PREFIX}empresa_value`,
            'eq',
            st
          );
        });
    ...
    if (filtro?.typeProgramId && filtro.typeProgramId.length) {
      f.or((p) => {
        filtro.typeProgramId.forEach((st) => {
          p.filterExpression(
            `${PREFIX}Programa/_${PREFIX}tipoprograma_value`,
            'eq',
            st
          );
        });
    ...
    if (filtro?.programTemperatureId && filtro.programTemperatureId.length) {
      f.or((p) => {
        filtro.programTemperatureId.forEach((st) => {
          p.filterExpression(
            `${PREFIX}Programa/_${PREFIX}temperatura_value`,
            'eq',
            st
          );
        });
...
export const buildItem = (item) => {
  const res = {
    ...
    [`${PREFIX}excluirtarefas`]: item?.deleteTask ? 'Sim' : 'Não',
