...
export const buildQuery = (filtro) => {
    var query = new QueryBuilder().filter((f) => {
        ...
        (filtro === null || filtro === void 0 ? void 0 : filtro.searchQuery) &&
            f.or((p) => {
                ...
                p.filterPhrase(`contains(${PREFIX}nomepreferido,'${filtro.searchQuery}')`);
                p.filterPhrase(`contains(${PREFIX}email,'${filtro.searchQuery}')`);
                p.filterPhrase(`contains(${PREFIX}emailsecundario,'${filtro.searchQuery}')`);
                p.filterPhrase(`contains(${PREFIX}celular,'${filtro.searchQuery}')`);
                p.filterPhrase(`contains(${PREFIX}EscolaOrigem/${PREFIX}nome,'${filtro.searchQuery}')`);
                p.filterPhrase(`contains(${PREFIX}Titulo/${PREFIX}nome,'${filtro.searchQuery}')`);
                return p;
            });
    ...
    // );
    query.expand(`${PREFIX}Titulo,${PREFIX}Pessoa_Etiqueta_Etiqueta,${PREFIX}EscolaOrigem,${PREFIX}AreaResponsavel,${PREFIX}Pessoa_AreaResponsavel`);
    if (filtro.top) {
