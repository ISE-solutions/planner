...
const ActivityGroupModelPage = ({ context }) => {
    const [scheduleChoosed, setActivityGroupChoosed] = React.useState();
    ...
    React.useEffect(() => {
        if (modelIdParam) {
            getScheduleId(modelIdParam).then(({ value }) => setActivityGroupChoosed(value[0]));
        }
    }, [modelIdParam]);
    React.useEffect(() => {
    ...
    return (React.createElement(Page, { context: context, itemsBreadcrumbs: [
        ...
        React.createElement(AccordionVertical, { defaultExpanded: true, title: 'Modelos de Agrupamento de Atividades', width: 290, expansibleColumn: React.createElement(ListModels, { context: context, refetch: refetch, currentUser: currentUser, filter: filter, canEdit: canEdit, setFilter: setFilter, schedule: schedule, loading: loading, scheduleChoosed: scheduleChoosed, handleSchedule: (sch) => setActivityGroupChoosed(sch) }) }, scheduleChoosed ? (React.createElement(CalendarDay, { canEdit: canEdit, scheduleChoosed: scheduleChoosed, refetch: refetch })) : (React.createElement(Box, { display: 'flex', height: '100%', width: '55rem', justifyContent: 'center', alignItems: 'center' },
...
//# sourceMappingURL=ActivityGroupModelPage.js.map